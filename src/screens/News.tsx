...
const  News = () => {
  ...
  const ItemView = ({item, index}) => {
    ...
    return (
      <>
        <StyledLockerInfoFrameWrapper key={item.id} onPress={() => toggleModal(item.comments.length)}>
          <StyledLockerContentWrapper>
            ...
            <CommentBar onPress={() => toggleModal(item.comments.length)}>
              ...
              <CommentText> {item.comments.length} {item.comments.length > 1 ? 'komentarze' : item.comments.length < 1 ? 'komenatrzy' : 'komentarz'} </CommentText>
            </CommentBar>
          {modalVisible && <Comments comments={item.comments} />}
        </StyledLockerInfoFrameWrapper>
    {newsData.length - 1 === index && (
      <Button onPress={handleLogout} title={'Wyloguj'} />
    )}
    </>
      );
  };
  ...
  return (
    ...
    {isFetchingData && Platform.OS !== 'ios' ? (
        ...
        <StyledFlatlist data={newsData} keyExtractor={(item, index) => index.toString()} renderItem={ItemView} />
    )}
  </StyledContainer>
  );
}
